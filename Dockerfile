...
RUN apt-get install sudo vim git python-pip -y
...
RUN sudo apt-get install python-pip xvfb xserver-xephyr libxml2-dev libxslt-dev -y
RUN export PYCURL_SSL_LIBRARY=gnutls 
RUN sudo  apt-get install postgresql-server-dev-all -y
RUN sudo  apt-get install libcurl4-openssl-dev -y
RUN sudo  apt-get install proxychains -y
RUN rm v1.0.1.tar.gz
RUN wget "http://www.net-square.com/zip folders/httprint_linux_301.zip"
RUN apt-get install unzip -y
RUN unzip *.zip
RUN rm -f httprint_linux_301.zip
...
RUN unzip *.zip
RUN rm BIG-IP_cookie_decoder.zip
...
RUN unzip SV*.zip -d owtf/dictionaries/restricted/svndigger/
RUN rm -f SVNDigger.zip
RUN mkdir -p owtf/dictionaries/restricted/raft 
...
RUN mkdir -p owtf/dictionaries/restricted/combined
